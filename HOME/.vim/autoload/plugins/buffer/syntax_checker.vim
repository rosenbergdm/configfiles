...
function! plugins#buffer#syntax_checker#PluginSyntaxChecker(p)
  ...
  let ft['yaml'] = {
        ...
        \  'run_in_background' : 0,
        \  'ef' : 'string:%f:%l:%c:%m',
        \  'cmd' :['ruby','-r', 'yaml', '-e', library#Function('return "f=\"".expand("%")."\"; begin; YAML::load( File.open(f) ); rescue => e; print e.to_s.sub(/syntax error on line (\\d*), col (\\d*).*/,\"#{f}:\\\\1:\\\\2:yaml syntax error\"); end"')],
        \  'active' : 1
        \ }
  " java app is slow so run this in background
  let ft['css'] = {
        \  'pattern' : '*.css',
        \  'run_in_background' : 1,
        \  'ef' : 'string:%+PURI\ \:\ file:%f,\ Line\ \:\ %l\ %m',
        \  'cmd' :['css-validator',library#Function("return 'file://'.expand('%:p')")],
        \  'active' : 1
        \ }
